    ...
    buildMenu: function(){		
        ...
        Ext.apply(this, {
            items: [{
                ...
                handler: function(){
            	     var _this = this;
            	     if(this.rec.json == undefined){
            	    	  _this.fireEvent('eventdelete', _this, _this.rec, _this.ctxEl);
            	    	 
            	     }else{
                       var logId = this.rec.json.logId;
                       var cid = this.rec.json.cid;
         			   if(cid != 2){
         				alert("场地在使用中！不能删除");
         				return false;
         			  }else{
    				    Ext.MessageBox.confirm('删除提示', '确定要删除不对外开放时间吗？', function(c) {
    					   if(c=='yes'){
    						 Ext.Ajax.request({
    							   type : "POST",
    						       url : "SiteInfo_deleteIfOpeTimeLog.do",
    						       params:{ opeId:logId},
    						       async: false,
...
Ext.ensible.cal.EventEditWindow = Ext.extend(Ext.Window, {
    ...
    onRender : function(ct, position){
        ...
        if(this.calendarStore){		
			var s = this.calendarStore;
			var records = []; 
			records.push(this.calendarStore.getAt(1).copy());
			var store2 = new Ext.data.Store({ recordType: this.calendarStore.recordType });
			store2.add(records);
...
alert('End: '+bounds.end);
    ...
    showEventEditor : function(rec, animateTarget){
		if(!rec.data){
			if(rec.EndDate){
				var e = new Date(rec.EndDate);
				var s = new Date(rec.StartDate);
				var ed = e.getFullYear()+'-'+e.getMonth()+"-"+e.getDate();
				var sd = s.getFullYear()+'-'+s.getMonth()+"-"+s.getDate();
				if(sd != ed){
					return;
				}
			...
			var cid = rec.json.cid;
			if(cid == 2){
				alert("场地不对外开放！无法查看数据");
				return false;
			}else{
		      var logId  =  rec.json.logId;
		      Ext.Ajax.request({
			           type : "POST",
				       url : "SiteInfo_findSingleLog.do",
				       params:{ logId:logId},
				       async: false,
				       success : function(o) { 
				        		...
				        		items:[{
				        			...
				        			items:[{
				        				...
				        				items:[{
				        					...
				        					fieldLabel:'联系电话',
				        					style:'background:none; border-right: 0px solid;border-top: 0px solid;border-left: 0px solid;border-bottom: #000000 0px solid;',
				        					value:data.data.relationtel
				        				},{

						        			xtype:'textfield',
						        			name:'activityname',
						        			fieldLabel:'活动名称',
						        			value:data.data.activityname,
						        			readOnly:true,
						        			style:'background:none; border-right: 0px solid;border-top: 0px solid;border-left: 0px solid;border-bottom: #000000 0px solid;',
						        			height:20
				        			...
				        			fieldLabel:'活动内容',
				        			value:data.data.activitycontent,
				        			style:'border:0', 
				        			readOnly:true,
				        			anchor: '97.5%',
				        			height:50
