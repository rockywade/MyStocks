...
Ext.ensible.cal.EventEditWindow = Ext.extend(Ext.Window, {
    ...
    onRender : function(ct, position){
        ...
        if(this.calendarStore){		
			...
			var records = []; 
			records.push(this.calendarStore.getAt(0).copy());
...
Ext.ensible.cal.CalendarPanel = Ext.extend(Ext.Panel, {
    ...
    initComponent : function(){
        ...
        if(this.showNavNextPrev){
            this.tbar.items.push([
                {id: this.id+'-tb-prev', handler: this.onPrevClick, scope: this, iconCls: 'x-tbar-page-prev',tooltip:'上周'},
                {id: this.id+'-tb-next', handler: this.onNextClick, scope: this, iconCls: 'x-tbar-page-next',tooltip:'下周'}
            ]);
        }
        if(this.showNavJump){
            this.tbar.items.push([
