...
Ext.onReady(function(){
	...
	var UuserObj = [
		{ name:'id', type:'int'},
		{ name:'orgaName', type:'string'},
		{ name:'orderNum', type:'int'}
	];
	
	var store = new Ext.data.JsonStore({
	    url: '/MyStock/orga_findPageOrga.do',
	    root: 'root',
	    totalProperty: 'total',
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        cm: new Ext.grid.ColumnModel({
			...
			columns:[new Ext.grid.RowNumberer(),
	            {header: '名称', width: 600,align:'center', dataIndex: 'orgaName'},
        	...
        	handler: function(){
				...
				if(!record){
					...
					Ext.MessageBox.confirm('删除提示', '是否删除该园区？', function(c) {
					   if(c=='yes'){
					   		Ext.Ajax.request({
					   			url : "/MyStock/orga_deleteOrga.do",
			...
			handler : function() {
				if (uForm.getForm().isValid()) {
					uForm.getForm().submit({
						url : '/MyStock/orga_saveOrUpdateOrga.do',
