...
Ext.onReady(function(){
    ...
    var grid = new Ext.grid.GridPanel({
        ...
        cm: new Ext.grid.ColumnModel({
			...
			columns:[sm,
			    {header: '标示',  width: 10, height:60,align:'center', dataIndex: 'id', hidden : true},
        ...
        tbar:['->',{
			...
			if(!record){
            	Ext.Msg.alert('信息提示','请选择需要置顶的二手书店');  
			}else{
				var jsonArray = [];
    		...
    		var record= grid.getSelectionModel().getSelected();
			if(!record){
            	Ext.Msg.alert('信息提示','请选择需要取消置顶的二手书店');  
			}else{
				var jsonArray = [];
				var recs=grid.getSelectionModel().getSelections(); 
				Ext.each(recs, function (item) {
    		...
    		var record= grid.getSelectionModel().getSelected();
			if(!record){
            	Ext.Msg.alert('信息提示','请选择需要取消隐藏的二手书店');  
			}else{
				var jsonArray = [];
				var recs=grid.getSelectionModel().getSelections(); 
				Ext.each(recs, function (item) {
    		...
    		var record= grid.getSelectionModel().getSelected();
			if(!record){
            	Ext.Msg.alert('信息提示','请选择需要高亮的二手书店');  
			}else{
				var jsonArray = [];
				var recs=grid.getSelectionModel().getSelections(); 
				Ext.each(recs, function (item) {
    		...
    		var record= grid.getSelectionModel().getSelected();
			if(!record){
            	Ext.Msg.alert('信息提示','请选择需要取消高亮的二手书店');  
			}else{
				var jsonArray = [];
				var recs=grid.getSelectionModel().getSelections(); 
				Ext.each(recs, function (item) {
